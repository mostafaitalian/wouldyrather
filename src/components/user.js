    ...
    render(){
        ...
        return(
            <Card style={{margin:'10px'}}>
                <Card.Header>
                    <span style={{display:'inline',marginRight:'5px'}}><img src={this.props.state.users[this.props.id].avatarURL} style={{borderRadius:'50%', width:'40px'}}/></span><h5 style={{float:'right'}}>{this.props.state.users[this.props.id].name}</h5>
                </Card.Header>
                <Card.Body style={{display:'flex',flexDirection:'row'}}>
                    ...
                    <Card.Text style={{marginLeft:'10px'}}>{user.name} has answered {Object.keys(user.answers).length} questions out of {answersUser}<br/><br/></Card.Text>
